...
## 6. Próximo día
 ...
 - [ ] **Importar los 155 PDFs listos al catálogo** (desde `PDFs/listos_para_subir/{Historia,Teoria}/`) con `@content-importer` + `@evento-builder`: fichas con `pdftotext`, `filename` → contenedor, verificación HTTP 200, tarjetas de evento para los históricos. Revisar primero las notas del inventario (solapamientos con tarjetas existentes: Comuna de París, Durruti, Mujeres Libres, 1º de Mayo, Sacco y Vanzetti…).
