...
const TheoriesView = ({ darkMode, regionData, onRead = () => {}, onOpenLibrary = () => {} }) => {
  ...
  return (
    <div>
                ...
                <div className="mt-4 space-y-3">
                  ...
                  {books.length > 0 && (
                    <div className="space-y-2">
                      {books.map((book, idx) => (
                        <div key={`${theory.id}-${idx}`} className={`rounded-lg border p-3 ${darkMode ? 'bg-gray-800/60 border-gray-700/50' : 'bg-white/80 border-amber-300'}`}>
      ...
      <div className={`mt-8 flex items-start gap-3 rounded-lg border-2 p-4 ${darkMode ? 'bg-gray-900/40 border-gray-700/50' : 'bg-white/60 border-amber-300'}`}>
